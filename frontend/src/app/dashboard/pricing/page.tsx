      ...
      <main className="flex-1 ml-64 p-6">
        ...
        <div className="text-center mb-10">
          ...
          <p className="text-sm text-muted-foreground mt-2">
            💎 Tingkatkan kemampuan trading AI — <span className="text-chart-5">プラン</span>
          </p>
        </div>
        ...
        {loading ? (
          ...
          <div className="text-center py-20">
            ...
            <p className="text-muted-foreground">Belum ada data paket berlangganan.</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 max-w-5xl mx-auto">
            {planKeys.map((key) => {
              ...
              const gradient = PLAN_GRADIENTS[name] || "from-slate-400 to-slate-600";
              const isCurrentPlan = user?.role === name || (user?.role === "free" && name === "free");
              ...
              return (
                ...
                >
                  ...
                  <CardHeader className="text-center pb-2">
                    <div className={`w-14 h-14 rounded-2xl mx-auto mb-3 bg-gradient-to-br ${gradient} flex items-center justify-center shadow-lg`}>
                      <Icon className="w-7 h-7 text-white" />
                    </div>
                    <CardTitle className="text-lg capitalize">{plan.name || key}</CardTitle>
                    <CardDescription>
                      {plan.daily_requests_limit ? `${plan.daily_requests_limit} requests/hari` : "Unlimited"}
                    </CardDescription>
                  </CardHeader>
