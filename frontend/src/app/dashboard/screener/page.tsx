...
export default function ScreenerPage() {
  ...
  const runScreener = async () => {
    ...
    try {
      ...
      const msg =
        err instanceof Error ? err.message : "Gagal menjalankan screener";
      setError(msg);
    } finally {
      setLoading(false);
    }
  ...
  const getActionBadge = (action?: string) => {
    ...
    if (a === "SELL" || a === "STRONG SELL")
      return (
        <Badge className="bg-trade-down/15 text-trade-down border-trade-down/20 hover:bg-trade-down/20 gap-1.5 px-2.5 py-1">
          <FontAwesomeIcon
            icon={faArrowTrendDown as IconProp}
            className="w-3 h-3"
          />{" "}
          {a}
        </Badge>
    ...
    return (
      <Badge
        variant="outline"
        className="border-white/10 text-muted-foreground gap-1.5 px-2.5 py-1"
      >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
              ...
              <div className="flex items-end">
                <p className="text-[10px] text-muted-foreground italic leading-tight">
                  Adjust these parameters to find high-probability setups across
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <CardDescription className="text-[10px]">
                Found <span className="text-primary font-bold">{count}</span>{" "}
                matched assets based on criteria
              </CardDescription>
            </div>
          </div>
        </CardHeader>
              ...
              <Table>
                <TableHeader className="bg-white/5">
                  <TableRow className="hover:bg-transparent border-white/10">
                    <TableHead className="font-black text-[10px] uppercase tracking-widest w-[150px]">
                      Symbol
                    </TableHead>
                    <TableHead className="font-black text-[10px] uppercase tracking-widest text-center">
                      Action
                    </TableHead>
                    <TableHead className="font-black text-[10px] uppercase tracking-widest text-right">
                      Confidence
                    </TableHead>
                    <TableHead className="font-black text-[10px] uppercase tracking-widest text-right">
                      Price
                ...
                <TableBody>
                  {results?.map((item, i) => (
                    <TableRow
                      ...
                      onClick={() =>
                        router.push(`/dashboard/market?symbol=${item.symbol}`)
                      }
                    >
                      ...
                      <TableCell className="text-right">
                        ...
                        >
                          {item.Confidence != null
                            ? `${(item.Confidence as number).toFixed(1)}%`
                            : "—"}
                        </span>
                      </TableCell>
                      ...
                      <TableCell colSpan={7} className="h-48 text-center">
                        <div className="flex flex-col items-center justify-center space-y-2 opacity-40">
                          ...
                          <p className="text-sm font-bold">
                            No assets matched your criteria.
                          </p>
                          <p className="text-xs">
                            Try loosening your scan parameters.
                          </p>
                        </div>
                      </TableCell>
                    </TableRow>
