...
import type { AdminUser, RevenueStats, UpgradeRequest, PipelineStatus } from "@/types";
...
export default function AdminPage() {
  ...
  }, [isAuthenticated, user, fetchUsers, fetchRevenue, fetchQueue, fetchPipeline]);
      ...
      <main className="flex-1 ml-64 p-6">
        ...
        <div className="mb-6">
          ...
          <p className="text-sm text-muted-foreground mt-1">
            👑 Manage users & revenue — <span className="text-chart-5">管理パネル</span>
          </p>
        </div>

        <Tabs defaultValue="overview" className="space-y-6">
          <TabsList className="glass-panel border border-white/10 p-1">
            <TabsTrigger value="overview" className="data-[state=active]:bg-primary/20 data-[state=active]:text-primary cursor-pointer">
              <BarChart3 className="w-4 h-4 mr-1.5" /> Overview
            </TabsTrigger>
            <TabsTrigger value="users" className="data-[state=active]:bg-primary/20 data-[state=active]:text-primary cursor-pointer">
              <Users className="w-4 h-4 mr-1.5" /> Users
            </TabsTrigger>
            <TabsTrigger value="queue" className="data-[state=active]:bg-primary/20 data-[state=active]:text-primary cursor-pointer">
              <Clock className="w-4 h-4 mr-1.5" /> Queue
              {queue.length > 0 && (
                <Badge className="ml-1.5 bg-trade-down/20 text-trade-down px-1.5 text-[10px]">{queue.length}</Badge>
              )}
            </TabsTrigger>
            <TabsTrigger value="pipeline" className="data-[state=active]:bg-primary/20 data-[state=active]:text-primary cursor-pointer">
                ...
                <>
                  ...
                  <Card className="glass-panel border-white/10">
                    <CardContent className="p-5 text-center">
                      ...
                      <p className="text-3xl font-bold text-trade-up">${revenue.monthly_revenue_usd}</p>
                      <p className="text-xs text-muted-foreground mt-1">Monthly Revenue</p>
                    </CardContent>
                  </Card>
                  {Object.entries(revenue.breakdown || {}).map(([role, count]) => (
                      ...
                      {filteredUsers.map((u) => (
                        <TableRow key={u._id} className="border-white/5 hover:bg-white/5">
                          ...
                          <TableCell className="text-right font-mono text-xs">
                            {u.requests_today ?? 0}/{u.daily_requests_limit ?? "∞"}
                          </TableCell>
                          <TableCell>
                            <Button
                              variant="ghost"
          ...
          <TabsContent value="queue">
            <Card className="glass-panel border-white/10">
              <CardHeader>
                ...
                <CardDescription>{queue.length} permintaan menunggu</CardDescription>
              </CardHeader>
              <CardContent className="p-0">
                {queue.length === 0 ? (
                  <p className="text-center text-muted-foreground py-8">Tidak ada permintaan.</p>
                ) : (
                  <Table>
                    <TableHeader>
                      <TableRow className="border-white/10 hover:bg-transparent">
                    ...
                    <TableBody>
                      {queue.map((req) => (
                        <TableRow key={req._id || req.id} className="border-white/5 hover:bg-white/5">
                          ...
                          <TableCell>
                            <Badge className={`${
                              req.status === "pending" ? "bg-chart-4/20 text-chart-4" : "bg-trade-up/20 text-trade-up"
                            }`}>
                              {req.status}
                            </Badge>
                          </TableCell>
                          ...
                          <TableCell>
                            <div className="flex gap-1">
                              ...
                              <Button variant="ghost" size="sm" className="text-trade-down hover:text-trade-down">
          ...
          <TabsContent value="pipeline">
            <Card className="glass-panel border-white/10 max-w-2xl">
              <CardHeader>
                <CardTitle className="flex items-center gap-2">
                  <Activity className="w-5 h-5 text-emerald-400" /> AI Retraining Pipeline
                </CardTitle>
                <CardDescription>Monitor and trigger neural engine optimizations and nightly training jobs.</CardDescription>
              </CardHeader>
              <CardContent className="space-y-6">
                <div className="p-4 bg-white/5 border border-white/10 rounded-lg">
                  <div className="flex items-center justify-between mb-2">
                    ...
                    {pipelineState ? (
                      <Badge className={
                        pipelineState.status === "running" ? "bg-emerald-500/20 text-emerald-400" :
                        pipelineState.status === "failed" ? "bg-red-500/20 text-red-500" :
                  ...
                  {pipelineState?.status === "running" && (
                    <div className="mt-4 space-y-2">
                      <div className="flex justify-between text-xs">
                        <span className="text-emerald-400 animate-pulse">{pipelineState.message || "Optimization in progress..."}</span>
                      ...
                      {pipelineState.eta && (
                        <p className="text-xs text-muted-foreground">ETA: {pipelineState.eta}</p>
                      )}
                    </div>
                  )}
                  ...
                  {pipelineState?.status !== "running" && (
                    <div className="mt-4 text-sm text-muted-foreground">
                      ...
                      {pipelineState?.last_run && <p className="mt-1">Last run: {new Date(pipelineState.last_run).toLocaleString()}</p>}
                    </div>
                  )}
                </div>
                ...
                <div className="pt-4 border-t border-white/10">
                  <Button 
                    ...
                    disabled={triggeringPipeline || pipelineState?.status === "running"}
                  ...
                  >
                    {triggeringPipeline ? <Loader2 className="w-4 h-4 mr-2 animate-spin" /> : <Zap className="w-4 h-4 mr-2" />}
                    Trigger Hyper-Optimization
                  </Button>
                  <p className="text-xs text-muted-foreground mt-3">
                    Warning: Manual trigger will immediately fetch new market conditions and adjust neural nets. This is extremely computationally expensive.
                  </p>
                </div>
              </CardContent>
            </Card>
        ...
        <Dialog open={showApprove} onOpenChange={setShowApprove}>
          <DialogContent className="glass-panel border-white/10">
            <DialogHeader>
              ...
              <DialogDescription>Upgrade {approveEmail} ke plan baru</DialogDescription>
            </DialogHeader>
            ...
            <DialogFooter>
              <Button variant="outline" onClick={() => setShowApprove(false)} className="border-white/10">
                Batal
              </Button>
              <Button onClick={approveUser} disabled={approving} className="bg-trade-up/20 text-trade-up hover:bg-trade-up/30">
