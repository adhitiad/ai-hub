...
class RiskManager:
    def __init__(self):
        self.MAX_DAILY_LOSS_PERCENT: float = float(
            os.getenv("MAX_DAILY_LOSS_PERCENT", "0.05")
        )
        self.MAX_CONSECUTIVE_LOSSES: int = int(os.getenv("MAX_CONSECUTIVE_LOSSES", "5"))
        self.SYSTEM_BALANCE: float = float(os.getenv("SYSTEM_BALANCE", "1000"))
...
async def check_circuit_breaker(
    balance: float, max_daily_loss_percent: float, max_consecutive_losses: int
):
    ...
    if daily_loss_amount >= max_loss_limit:
        return (
            False,
            f"CIRCUIT BREAKER: Daily Loss ${daily_loss_amount:.2f} > Limit ${max_loss_limit:.2f}",
        )

    # 2. Cek Loss Streak (Berturut-turut ke belakang)
    recent_cursor = (
